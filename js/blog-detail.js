...
async function renderArticle(slug){ console.debug('renderArticle called with slug:', slug); const posts = await window.getPosts(); const p = posts.find(x=>x.slug===slug); const container = document.getElementById('articleContainer'); if(!p){ container.innerHTML = `<div class="min-h-screen flex items-center justify-center px-2"><div class="text-center fade-in-up"><h2 class="text-4xl font-bold mb-4">Article not found</h2><p class="text-slate-400 mb-6">The article you're looking for doesn't exist.</p><a href="blog.html" class="btn-primary">Back to blog</a></div></div>`; return; }
        ...
        ${relatedPosts.length > 0 ? `
          <div class="mt-16 mb-8">
            ...
            <div class="grid md:grid-cols-2 gap-6">
              ${relatedPosts.map(rp => `
                <a href="blog-detail.html?post=${rp.slug}" class="related-article block group">
...
window.shareArticle = function(platform){ const url = window.location.href; const text = document.title; switch(platform){ case 'twitter': window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(text)}`, '_blank'); break; case 'facebook': window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`, '_blank'); break; case 'linkedin': window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}`, '_blank'); break; case 'copy': navigator.clipboard.writeText(url).then(()=> alert('Link copied to clipboard!')); break; } }

// search suggestions on detail page
(function(){ const input = document.getElementById('searchInputTop'); const box = document.getElementById('searchSuggestions'); input?.addEventListener('input', async (e)=>{ const q = e.target.value.trim().toLowerCase(); if(q.length <2){ box?.classList.add('hidden'); return; } const posts = await window.getPosts(); const items = posts.filter(p => p.title.toLowerCase().includes(q) || p.excerpt.toLowerCase().includes(q));
