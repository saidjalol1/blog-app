...
class SliderLatest {
  ...
  slideWidth(){ return this.items.length ? this.items[0].getBoundingClientRect().width : 0; }
  visibleCount(){ const sw = this.slideWidth(); if(!sw) return 1; const containerWidth = this.wrapper.parentElement.offsetWidth; return Math.round(containerWidth / sw); }
  maxIndex(){ return Math.max(0, this.items.length - this.visibleCount()); }
  update(){ if(!this.items.length) return; const translateX = this.index * this.slideWidth(); this.wrapper.style.transform = `translateX(-${translateX}px)`; }
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  try{
    const postsData = await window.getPosts();
